            ...
            pipeline: [
               {
                  $match: {
                     $expr: {
                        $and: [{ $in: [{ $literal: "Agency" }, "$role"] }, { $eq: ["$_id", "$$createdBy"] }],
                     },
                  },
               },
               {
            ...
            pipeline: [
               {
                  $match: {
                     $expr: {
                        $and: [{ $in: [{ $literal: "Operator" }, "$role"] }, { $eq: ["$_id", "$$createdBy"] }],
                     },
                  },
               },
               {
         ...
         $addFields: {
            createdByInfo: {
               $cond: [
                  ...
                  {
                     $cond: [{ $gt: [{ $size: "$createdByOperator" }, 0] }, { $arrayElemAt: ["$createdByOperator", 0] }, null],
                  },
               ],
            },
         },
...
const HandleUpdateTicketStatus = async (req, res, next) => {
   try {
      ...
      // Update ticket
      const updated = await SupportModel.findByIdAndUpdate(ticketID, { status }, { new: true });
...
export { HandleCreateSupport, HandleGetSupport, HandleGetSingleTicket, HandleUpdateTicketStatus };
