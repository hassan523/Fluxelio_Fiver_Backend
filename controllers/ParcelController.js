...
import { getParcelStatusMessage, getTransactionStatusMessage } from "../utils/TrackingStatus.js";
...
const HandleCreateParcel = async (req, res, next) => {
   ...
   try {
      ...
      const { weight, customerID, transportMethod, destinationID, tagID, estimateArrival, description, mixedPackage, whatsappNotif, pricePerKilo, actualCarrierCost, paymentStatus, status, createdBy, partialAmount } = req.body;
      ...
      // let packagePictures = req.files?.packagePicture || [];
      // console.log(req.files?.packagePicture);
      // if (packagePictures) {
      //    packagePictures = await Promise.all(
      //       req.files.packagePicture.map(async (file) => {
      ...
      if (req.files?.packagePicture) {
         packagePictures = await Promise.all(
            req.files.packagePicture.map(async (file) => {
               ...
               return uploadResult.secure_url; // 👈 THIS was missing
            }),
         );
      }

      console.log(packagePictures);
      ...
      const destinationOffice = await OfficeModel.findById(destinationID).session(session);
      ...
      const generateTrackingID = await GenerateTrackingID(findAgency.companyCode, destinationOffice.address.country, session);
      ...
      if (paymentStatus === "PARTIALLY PAID") {
         if (!partialAmount || partialAmount <= 0) {
            ...
            session.endSession();
            return res.status(400).json({
               message: "Partial amount is required and must be greater than 0 when payment status is PARTIALLY PAID",
            });
         }
      ...
      const statusValidation = validateParcelStatusByPayment(status, paymentStatus);
   ...
   try {
      ...
      pipeline.push({
         $project: {
            ...
            tagID: 1,
            tag: {
               $cond: [{ $eq: ["$tagID", null] }, null, { $arrayElemAt: ["$tagDetails", 0] }],
            },
            packagePicture: 1,
            createdBy: 1,
            createdByType: 1,
               ...
               pipeline: [
                  {
                     $match: {
                        $expr: {
                           $and: [{ $in: [{ $literal: "Admin" }, "$role"] }, { $eq: ["$_id", "$$updatedBy"] }],
                        },
                     },
                  },
               ...
               pipeline: [
                  {
                     $match: {
                        $expr: {
                           $and: [{ $in: [{ $literal: "Agency" }, "$role"] }, { $eq: ["$_id", "$$updatedBy"] }],
                        },
                     },
                  },
                  {
               ...
               pipeline: [
                  {
                     $match: {
                        $expr: {
                           $and: [{ $in: [{ $literal: "Operator" }, "$role"] }, { $eq: ["$_id", "$$updatedBy"] }],
                        },
                     },
                  },
                  {
                  ...
                  $cond: [
                     ...
                     { $arrayElemAt: ["$trackingUpdatedByAdmin", 0] },
                     {
                        $cond: [
                           ...
                           {
                              $cond: [{ $gt: [{ $size: "$trackingUpdatedByOperator" }, 0] }, { $arrayElemAt: ["$trackingUpdatedByOperator", 0] }, null],
                           },
                        ],
                     },
                  ],
                  ...
                  $cond: [
                     ...
                     { $arrayElemAt: ["$updatedByInfoAdmin", 0] },
                     {
                        $cond: [
                           ...
                           {
                              $cond: [{ $gt: [{ $size: "$updatedByInfoOperator" }, 0] }, { $arrayElemAt: ["$updatedByInfoOperator", 0] }, null],
                           },
                        ],
                     },
                  ],
...
const HandleUpdateParcelStatus = async (req, res, next) => {
   ...
   try {
      ...
      if (findParcel.tagID !== null) {
         return res.status(400).json({ message: "Tag Parcels Cannot Be Updated Individually" });
      }
      ...
      if (!findTransaction) {
         await session.abortTransaction();
         session.endSession();
         return res.status(404).json({ message: "Transaction not found for this parcel" });
      }

      let updatedPackagePictures = findParcel.packagePicture || [];

      if (req.files?.packagePicture?.length > 0) {
         ...
         updatedPackagePictures = [...updatedPackagePictures, ...newPictures];
      }
            ...
            $project: {
               ...
               updatedByUsername: {
                  $cond: [
                     ...
                     {
                        $cond: [{ $eq: ["$updatedByType", "Agency"] }, { $arrayElemAt: ["$updatedByAgency.agencyName", 0] }, { $arrayElemAt: ["$updatedByOperator.username", 0] }],
                     },
                  ],
               },
               partialAmount: 1,
...
export { HandleCreateParcel, HandleGetParcels, HandleGetTransaction, HandleGetSingleParcel, HandleGetAgencyParcels, HandleGetOfficeParcels, HandleUpdateParcelStatus, HandleBulkUpdateParcelStatus, HandleParcelReqFilters, HandleTrackParcel };
